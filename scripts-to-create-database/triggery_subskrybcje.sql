create trigger NOWA_SUBSKRYBCJA
    ...
    begin
     ...
     SELECT COUNT(*) INTO vCheckCount FROM ROWERY WHERE ID_ROWERU=:NEW.ID_ROWERU;
        ...
        if :OLD.ID_ROWERU IS NULL or :NEW.ID_ROWERU != :OLD.ID_ROWERU or
           (:OLD.DATA_ZAKONCZENIA is not null and :NEW.DATA_ZAKONCZENIA is null) then
...
create or replace trigger SUBSKRYBOWANIE
    after insert
    on SUBSKRYBCJE
    for each row
BEGIN
...
create or replace trigger ZAKONCZENIE_SUBSKRYBCJI
    after update
    on SUBSKRYBCJE
    for each row
BEGIN
     IF :NEW.DATA_ZAKONCZENIA IS NOT NULL AND :OLD.DATA_ZAKONCZENIA IS NULL THEN
